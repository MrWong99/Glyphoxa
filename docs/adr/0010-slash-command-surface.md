...
- **ACK:** the handler ALWAYS Defers ephemerally, so the "thinking…" placeholder is GM-only on every path and an error (always an ephemeral reply) never leaves a dangling PUBLIC placeholder. A public/voiced-degraded SUCCESS is delivered as a PUBLIC Followup, whose visibility is independent of the ephemeral placeholder. The recap runs under its own 120s timeout (the Defer stops the first-response watchdog); a slow recap surfaces a friendly "took too long" reply, other failures the generic one.
