...
- **The SDK owns SSE framing now.** The hand-rolled reader capped each SSE line (1 MiB) and the whole raw stream (16 MiB) as a guard against a hostile/misbehaving `WithBaseURL` gateway. The core originally kept the whole-stream guard as a **decoded**-bytes cap (summed content + tool-argument bytes, 16 MiB) — but that counted zero for content-less chunks, so an endless flood of empty frames never tripped it (#152). The guard now counts **every** chunk: its re-serialized JSON (`RawJSON()`, the closest raw-size approximation the SDK exposes, never smaller than the decoded content it carries) against the 16 MiB budget, plus a 100k chunk-count cap so tiny-frame floods terminate promptly. A single oversized SSE line is bounded by the SDK's own decoder rather than by us. Acceptable for the BYOK/self-hosted-gateway threat model.
