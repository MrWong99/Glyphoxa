...
**Amendment (2026-07-27, live beta): the stable prefix also carries the output-format rule.** Hot Context labels every human line `"Name: text"` so an Agent can tell its players apart, and the stable prompt's roster block teaches that grammar. Models imitate the grammar they are shown: live NPCs answered `"Mira: Good evening"`, and since the reply is read aloud verbatim, the NPC spoke its own name before every line. It compounded — the delivered text is what ADR-0012 commits to history and what the Chunker writes into the Transcript, so the next turn's context contained an ASSISTANT line in exactly that shape and the model copied itself.
