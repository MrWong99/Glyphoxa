...
## Trigger — confirm window, not fast onset

Utterance *capture* already fires fast (Silero `minSpeechFrames`, ~90ms at 30ms frames). Floor *yielding* uses a second, slower knob: a **barge-in confirm window** (default ~250ms of continuous voiced speech, measured from `speech_start`). Speech that ends before the window is **soft-overlap** (a backchannel — "mhm", "yeah", a cough); it emits an observability event but does **not** cancel the Agent. Speech that crosses the window emits `barge.detected` and cancels.
