...
**Pure function over a swappable index.** Each `TargetMatch` call is a pure function over the fuzzy name index — no vendor and no LLM in the loop — so the whole chain is unit-testable (ADR-0019, ADR-0021). The roster is **dynamic**, not fixed at construction: `Matcher.Add` and `Matcher.Remove` bring a Character NPC into or out of the scene mid-Voice Session. They rebuild the fuzzy index under the matcher's mutex and **atomically swap** it in (an `atomic.Pointer[fuzzyIndex]`), so `TargetMatch` reads the index lock-free and a concurrent roster change can never tear a scoring pass — a pass sees the whole old index or the whole new one, never a half-built one. The atomic swap is the matcher's only mutation point on the scoring path; each scoring call remains pure against whichever index it loaded. (`Remove` also prunes the departed Agent's last-addressed and interruption state so a later unnamed continuation cannot resurrect it.)
