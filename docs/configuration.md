...
| `GLYPHOXA_VOICE_MEDIA_STALL_WINDOW` | optional | How long inbound voice RTP may be absent — while participants keep announcing speech on the voice websocket — before the media watchdog declares the Discord media path dead and rebuilds the voice connection ([ADR-0064](adr/0064-voice-media-path-liveness.md), #633). Go duration; default `45s`; doubled while a fresh connection has yet to carry its first packet. The literal `off` disables the verdict (the once-a-minute liveness log keeps running); an unparsable value takes the default. A quiet table can never trip it — silence without Speaking events is not a stall. |
