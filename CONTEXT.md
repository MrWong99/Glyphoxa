...
| **Butler** | The Campaign-scoped Agent (`agent_role=butler`) that assists the GM via slash commands, in-voice address, and Tool calls; exactly one per Campaign, auto-created and undeletable (ADR-0009). **Address-Only** and **not part of the voiced Cast in v1** — never in the address Matcher/Cast, so it is not a mute target. Only partially editable: name/title/persona/voice/aliases mutable; `agent_role` and Address-Only pinned. | Assistant, Helper, GM agent |
