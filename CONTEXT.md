...
| **Session-scoped attribution** | The rule that every voice-event-bus event carries its `SessionID`; consumers resolve session context from the event itself, never from a process-global snapshot (additive amendment to ADR-0020, landed with ADR-0057's multi-session concurrency). | Global snapshot (the superseded pattern) |
...
| **Voice Instance** | A process running in `voice` Mode that claims and hosts Voice Sessions via the `voice_sessions` Postgres table. A pool of Voice Instances is the "voice-worker pool" (ADR-0057); a single process is always a Voice Instance, never "a worker". | Worker (v1 term, deprecated in v2) |
