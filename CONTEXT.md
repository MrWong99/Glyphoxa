...
| **Campaign Language** | The natural language a Campaign is played in; selects the phonetic scheme used by Address Detection's name matching and the language hint for STT/TTS. Surfaced to the GM as "Spoken language". | Locale, Lang, Display Language |
| **Display Language** | The language the web app's own chrome is rendered in (`en`/`de`), a pure web-tier preference stored in `localStorage` and served by the typed catalog under `web/src/i18n`. It drives *everything* the web tier renders: message strings AND every locale-sensitive format — dates, clocks, grouped numbers, currency amounts — which take it explicitly rather than defaulting to the browser's locale. It never touches STT/TTS or Address Detection; that is the Campaign Language. | Locale, Language (unqualified), Campaign Language |
...
| **Planning Thread** | One persisted conversation of the Butler planning chat (#592, ADR-0062): the GM-only web chat where the Campaign's Butler assists prep between sessions. Campaign-scoped, exported in the Campaign Bundle (format v3), deleted with the Campaign. Its messages are the prose turns only — tool-call intermediates are never persisted. | Chat (unqualified), Conversation, Session (reserved) |
...
| **Appearance** | A record that a KG Node was named in a committed Transcript Line — the "when did we last see that ogre" index (ADR-0008 fifth amendment). Deliberately NOT an Edge: an Edge is world truth an NPC's Hot Context walks, so a mention modelled as one would become a fact the NPC recites. Retrieval only; never prompt-facing. | Mention, Sighting, Reference, Link (reserved for Edge) |
...
| **Party Marker** | A Voice Session's current position — a Map plus optionally a Pin or a free `(x, y)`. Session-scoped, so concurrent sessions never see each other's; it is the one piece of map data that reaches an NPC's prompt, as a single bounded clause in the volatile tail (ADR-0059 amendment, ADR-0060). | Position, Cursor, Marker (unqualified) |
...
| **Address Detection** | The deterministic chain deciding which Agent(s), if any, a Transcript utterance targets: fuzzy name/alias match → last-speaker continuation (inside the Continuation Window) → single-NPC corroboration → no target. Only an explicit name, or a continuation of a live exchange, addresses on its own; pure Backchannel routes nowhere (ADR-0024, 2026-07-27 amendment). Returns a set; multiple targets trigger an Ensemble Turn. | Routing, Targeting |
| **Address-Only** | An Agent reachable only by explicit name/alias, excluded from every ambient signal (last-speaker continuation, single-NPC corroboration, expertise). The Butler is Address-Only by default; Character NPCs are not. | Wake-word-only, Explicit-only |
| **Continuation Window** | How long a last-speaker continuation claim survives: inside it an unnamed follow-up stays with the Agent just addressed, past it the exchange is over and the next utterance must name someone. Default 30s; a Barge-in revokes the claim at once (ADR-0024/ADR-0027). | Continuation timeout, Stickiness |
| **Backchannel** | An utterance that is entirely filler *vocalization* ("mhm", "hm", "äh", "aha") and names no Agent. It is transcribed and committed like any other utterance but addresses nobody, so it never opens a turn. A one-word ANSWER ("Ja.", "Nein.") is deliberately not a Backchannel — it carries the whole content of a reply. Distinct from Soft-overlap, which is about the DURATION of a voiced burst against the Barge-in Confirm Window, not about what was said. | Filler, Acknowledgement |
...
| **Default Voice Channel** | The linked Guild's stored voice channel (`deployment_config.voice_channel_id`) a Voice Session joins when its start carries no explicit channel pick — the Session screen's picker pre-selects it ("Set as default" writes it), `/glyphoxa start` always uses it, and re-linking a different Guild clears it. | Configured channel, Voice channel (unqualified), Fallback channel |
| **Voice Session** | The Tenant's resolved Bot's presence in one Discord voice channel, bound to a (Guild, Campaign, GM) tuple at start and hosted by exactly one Voice Instance. The joined channel is the start's explicit pick, else the Default Voice Channel. | Session (alone), Call, Live session |
| **Idle Close** | The hosting Voice Instance ending a Voice Session that processed no audio for the **Idle Close Window**, ran through more Discord connect cycles than its ceiling allows, or was the quietest session on an instance over a configured resource ceiling. A deliberate policy stop, not a fault — the row closes `ended` with an `end_reason` prefix, exactly as the ADR-0046 hard spend cap does (ADR-0061). When the **Media Watchdog** flagged the silence as a dead transport, the same idle breach closes `media_path_dead` instead of `idle_no_audio` (ADR-0064) — the table was not quiet, the pipe was. Distinct from a Speaker Lane's idle reap (ADR-0050), which retires ONE lane inside a still-live Voice Session. | Idle timeout (unqualified), Session timeout, Reaper, AFK kick, Suspension (reserved for web-identity revocation) |
| **Idle Close Window** | How long a Voice Session may process no audio before Idle Close ends it. Default 15 min, per deployment. "Audio" is an inbound room-audio frame or the Bot being asked to speak (`/say`, voiced recap, Highlight replay) — never Discord's Opus silence frames, which signal that a speaker STOPPED, and never the synthesized silence clock, which ticks through an empty channel forever. | Idle timeout, Grace period, Silence window (collides with the silence clock) |
| **Media Watchdog** | The per-connect-cycle guard over a Voice Session's inbound media path (ADR-0064, #633): when the voice UDP socket goes silent for the stall window despite evidence it should be receiving — the server's keepalive echoes ceasing on a socket that had them, or a participant announcing speech with no RTP following — it ends the cycle so the reconnect loop rebuilds the voice connection, and flags the session so a close that lands anyway reads `media_path_dead`. A genuinely quiet table can never trip it. Distinct from Idle Close (session-ending policy) — the Media Watchdog only rebuilds. | Liveness watchdog, Stall detector, Deaf detector |
...
| **Soft-overlap** | A participant's voiced burst shorter than the Barge-in Confirm Window ("mhm", "yeah", a cough). Does not cancel the Agent, but is still transcribed and committed normally. See Backchannel for the text-side rule it usually coincides with. | Filler, Short burst |
...
- A **Voice Session** binds (**Guild**, voice channel, **GM**, **Campaign**) and is hosted by exactly one **Voice Instance**, which may end it by **Idle Close**.
...
- **"Idle"** is overloaded across three grains, and they retire different things. A **Speaker Lane** idle-reaps (ADR-0050) — one departed speaker's VAD path goes, the Voice Session lives on. An **Idle Close** (ADR-0061) ends the whole **Voice Session**. And the Session screen calls a Tenant with no live session "idle" — a UI state, not a mechanism. Always qualify: "lane reap", "Idle Close", "no live session". Never say a Voice Session was "reaped" — in the claim plane that word means a stale heartbeat marked its Voice Instance dead (ADR-0057), which is the opposite diagnosis.
