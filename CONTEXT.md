...
| **Idle Close** | The hosting Voice Instance ending a Voice Session nothing is using: one that processed no audio for the **Idle Close Window**, ran through more Discord connect cycles than its ceiling allows, or was the quietest session on an instance over a configured resource ceiling. A deliberate policy stop, not a fault — the row closes `ended` with an `end_reason` prefix, exactly as the ADR-0046 hard spend cap does (ADR-0061). Distinct from a Speaker Lane's idle reap (ADR-0050), which retires ONE lane inside a still-live Voice Session. | Idle timeout (unqualified), Session timeout, Reaper, AFK kick, Suspension (reserved for web-identity revocation) |
