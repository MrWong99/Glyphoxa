...
| **Session-scoped attribution** | The rule that every voice-event-bus event carries its `SessionID`; consumers resolve session context from the event itself, never from a process-global snapshot. Decided by ADR-0057; lands with the session-scoped events slice (#487). | Global snapshot (the superseded pattern) |
