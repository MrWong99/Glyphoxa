...
func TestBuildConversation_RoutesEachKeyToItsAdapter(t *testing.T) {
	...
	_, _, cleanup, err := buildConversation(voiceevent.NewBus(), log,
		[]npcSpec{hardcodedNPC()}, "", ttseleven.New(""), nil, keys, "", false, nil, nil, nil, nil, nil, tool.Deps{}, nil, nil)
	if err != nil {
...
func TestBuildConversation_DispatchesOffProviderID(t *testing.T) {
	...
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	_, _, cleanup, err := buildConversation(voiceevent.NewBus(), log,
		[]npcSpec{hardcodedNPC()}, "", ttseleven.New(""), nil, providerKeys{}, "anthropic", false, nil, nil, nil, nil, nil, tool.Deps{}, nil, nil)
	if err != nil {
		t.Fatalf("buildConversation: %v", err)
	}
	defer cleanup()
