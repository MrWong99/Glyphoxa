...
// request to Gemini's OpenAI-compatibility endpoint and returns a channel of
...
// failure closes the channel early instead, matching the anthropic and tts
// adapters.
func (c *Client) Complete(ctx context.Context, req llm.Request) (<-chan llm.StreamEvent, error) {
	if c.apiKey == "" {
		return nil, fmt.Errorf("gemini.Complete: missing API key (set %s or pass it to New)", APIKeyEnv)
	}

	messages := toWireMessages(req.Messages)
	if len(messages) == 0 {
		return nil, fmt.Errorf("gemini.Complete: request has no messages")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("gemini.Complete: marshal body: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("gemini.Complete: build request: %w", err)
	}
...
// anthropic and elevenlabs adapters' error shape.
func readErrorResponse(resp *http.Response, op string) error {
	snippet, _ := io.ReadAll(io.LimitReader(resp.Body, 512))
	return fmt.Errorf("gemini.%s: HTTP %d %s: %s",
